...
describe('Device removal / insertion.', ()=>{
  ...
  before(()=>{
    return Helper.i18next_t('DeviceList.selectDevice').then(_=>{
      ...
      NO_DEVICES = _;
      return Actions.DeviceList.click().waitForVisible('button[data-id="deviceList"]+.dropdown-menu');
    });
  });

  it('Initially, one device is displayed.', ()=>{
    return expect(app.client.getText('button[data-id="deviceList"]+.dropdown-menu a span:nth-child(1)'))
  ...
  after(()=>{
    return Helper.MTPMock_AddDevice('0000-0000-0000-0000').then(()=>{
      return Actions.DeviceList.click().then(()=>{
        return Helper.wait_for_unvisible('button[data-id="deviceList"]+.dropdown-menu');
      })
    });
  });
});
